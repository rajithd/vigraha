...
import vigraha.dataloader.util.USSDTransactionProcessorThread;
...
public class USSDUploadManagerServiceImpl implements UploadManagerService {

    private static final Logger logger = LoggerFactory.getLogger(USSDUploadManagerServiceImpl.class);
    ...
    public USSDUploadManagerServiceImpl(int poolSize) {
        this.poolSize = poolSize;
        pool = Executors.newFixedThreadPool(this.poolSize);
    }

    public void upload() {
        logger.info("USSD CDR logs upload scheduler started");
        fileHandler = new FileHandler();
        String[] files = getFiles();
        logger.info("Total of [{}] USSD logs found", files.length);
        ...
        logger.info("USSD CDR logs upload scheduler completed");
        ...
        try {
            ...
            while (charBuffer.hasRemaining()) {
                ...
                if (character == rawBreaker.charAt(0)) {
                    pool.execute(new USSDTransactionProcessorThread(builder.toString(),
